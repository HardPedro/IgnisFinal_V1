...
import { db, initializeServerAuth } from './server/firebase.js';
...
async function createOrUpdateQuoteRequest(tenantId: string, convId: string, customerPhone: string, customerName: string, args: any, baseUrl: string) {
  let customerId = null;
  ...
  let replyText = `Certo! Registrei a sua solicitação: "${args.description || 'Orçamento'}".\n\nPor favor, confirme se o escopo está correto clicando no link abaixo antes de enviarmos para a oficina:\n${approvalLink}`;
  
  try {
    const aiSettingsRef = doc(db, `tenants/${tenantId}/settings`, 'ai_assistant');
    const aiSettingsSnap = await getDoc(aiSettingsRef);
    if (aiSettingsSnap.exists() && aiSettingsSnap.data().templateQuoteRequest) {
      replyText = aiSettingsSnap.data().templateQuoteRequest.replace('{link}', approvalLink);
    }
  } catch (err) {
    console.error('Error fetching ai settings for quote request template:', err);
  }
...
Histórico da conversa:\n`;
    ...
    if (functionCalls && functionCalls.length > 0) {
      ...
      if (call.name === 'generateQuoteRequest') {
        ...
        try {
          replyText = await createOrUpdateQuoteRequest(tenantId, convId, customerPhone, customerName, args, baseUrl);
        } catch (err) {
