...
export function IntelligentAssistant() {
  ...
  const [templateQuoteRequest, setTemplateQuoteRequest] = useState('Sua solicitação foi registrada, por favor confirme no link abaixo:\n{link}');
  const [templatePreQuote, setTemplatePreQuote] = useState('Aqui está o seu pré-orçamento detalhado. Por favor, confira os valores e confirme a aprovação clicando no link abaixo:\n{link}');
  const [templateQuote, setTemplateQuote] = useState('Aqui está o seu orçamento oficial. Por favor, confira os valores e confirme a aprovação clicando no link abaixo:\n{link}');
    ...
    const fetchSettings = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (data.templateQuoteRequest) setTemplateQuoteRequest(data.templateQuoteRequest);
          if (data.templatePreQuote) setTemplatePreQuote(data.templatePreQuote);
          if (data.templateQuote) setTemplateQuote(data.templateQuote);
      ...
      <div className="space-y-8">
        <div className="bg-white p-8 rounded-[32px] border border-gray-100 shadow-sm">
            ...
            <div className="space-y-6">
              <div>
                ...
                <textarea
                  ...
                  className="block w-full border border-gray-300 rounded-xl shadow-sm py-3 px-4 focus:outline-none focus:ring-yellow-500 focus:border-yellow-500 text-base sm:text-sm"
                />
              </div>
            </div>
                ...
                <div className="mb-4 border border-indigo-100 rounded-2xl overflow-hidden bg-indigo-50/30 flex flex-col h-[400px]">
                  ...
                  <div className="p-3 bg-white border-t border-gray-200 flex items-center">
                    <input 
                      ...
                      className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 text-base sm:text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    />
                    <button 
          ...
          <div className="mt-8 pt-8 border-t border-gray-100">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Mensagens de Confirmação (Pré-prontas)</h3>
            <p className="text-sm text-gray-500 mb-6">
              Configure as mensagens que serão enviadas junto com os links de aprovação. Use a variável <code>{'{link}'}</code> onde o link deve aparecer.
            </p>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Solicitação de Orçamento</label>
                <textarea
                  rows={4}
                  value={templateQuoteRequest}
                  onChange={(e) => setTemplateQuoteRequest(e.target.value)}
                  className="block w-full border border-gray-300 rounded-xl shadow-sm py-3 px-4 focus:outline-none focus:ring-yellow-500 focus:border-yellow-500 text-base sm:text-sm"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Pré-orçamento</label>
                <textarea
                  rows={4}
                  value={templatePreQuote}
                  onChange={(e) => setTemplatePreQuote(e.target.value)}
                  className="block w-full border border-gray-300 rounded-xl shadow-sm py-3 px-4 focus:outline-none focus:ring-yellow-500 focus:border-yellow-500 text-base sm:text-sm"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Orçamento Oficial</label>
                <textarea
                  rows={4}
                  value={templateQuote}
                  onChange={(e) => setTemplateQuote(e.target.value)}
                  className="block w-full border border-gray-300 rounded-xl shadow-sm py-3 px-4 focus:outline-none focus:ring-yellow-500 focus:border-yellow-500 text-base sm:text-sm"
                />
              </div>
            </div>
