...
export function Quotes() {
  ...
  const handleSendWhatsApp = async (quote: any) => {
    ...
    if (aiSettings?.templateQuote) {
      message = aiSettings.templateQuote
    ...
    try {
      ...
      if (customerSnap.exists()) {
        ...
        if (customerPhone) {
          ...
          if (isPreQuote && aiSettings?.templatePreQuote) {
            message = aiSettings.templatePreQuote
              .replace(/{link}/g, approvalLink)
          ...
          } else if (!isPreQuote && aiSettings?.templateQuote) {
            message = aiSettings.templateQuote
              .replace(/{link}/g, approvalLink)
              .replace(/{nome_cliente}/g, customerName)
              .replace(/{veiculo}/g, vehicleName)
          ...
          // Update status to 'enviado'
          const docRef = doc(db, `tenants/${tenantId}/${isPreQuote ? 'pre_quotes' : 'quotes'}`, id);
          await updateDoc(docRef, { status: 'enviado' });
        }
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        // Create a new quote from the pre-quote
        const quotesRef = collection(db, `tenants/${tenantId}/quotes`);
        const newQuote = await addDoc(quotesRef, {
          ...quoteData,
          preQuoteId: activePreQuoteId,
          createdAt: serverTimestamp()
        });
        
        // Update the pre-quote status to 'convertido'
        ...
        if (window.confirm('Você quer solicitar confirmação do cliente? (é altamente recomendado)')) {
          await sendApprovalMessage(tenantId, newQuote.id, quoteData, false);
        }
        setActiveTab('quotes');
      } else {
        const quotesRef = collection(db, `tenants/${tenantId}/quotes`);
                ...
                <li key={preQuote.id} className="p-6 hover:bg-gray-50 transition-colors">
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-2">
                        <span className={`px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full ${
                          (preQuote.status === 'aprovado' || preQuote.status === 'convertido') ? 'bg-green-100 text-green-800' :
