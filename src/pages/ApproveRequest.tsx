...
export function ApproveRequest() {
  ...
  if (success || request?.status === 'pendente' || request?.status === 'cancelado_pelo_cliente' || request?.status === 'em_andamento' || request?.status === 'concluido') {
    const isConfirmed = request?.status === 'pendente' || request?.status === 'em_andamento' || request?.status === 'concluido';
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center">
          {isConfirmed ? (
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-2">
            {isConfirmed ? 'Solicitação Confirmada!' : 'Solicitação Cancelada'}
          </h2>
