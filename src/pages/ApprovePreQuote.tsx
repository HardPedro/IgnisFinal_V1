...
export function ApprovePreQuote() {
  ...
  if (success || preQuote?.status === 'aprovado' || preQuote?.status === 'reprovado' || preQuote?.status === 'convertido') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center">
          {preQuote?.status === 'aprovado' || preQuote?.status === 'convertido' ? (
            <CheckCircle className="h-12 w-12 text-green-500 mx-auto mb-4" />
          ) : (
            <XCircle className="h-12 w-12 text-red-500 mx-auto mb-4" />
          )}
          <h2 className="text-xl font-bold text-gray-900 mb-2">
            Pré-orçamento {preQuote?.status === 'aprovado' || preQuote?.status === 'convertido' ? 'Aprovado' : 'Reprovado'}
          </h2>
